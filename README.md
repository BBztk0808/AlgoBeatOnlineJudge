...
> **当前版本**:v1.8.2 · 权限与安全收口 + 提交状态快照恢复 + 本地运行排障

近期主要里程碑(完整历史见 [#版本历史](#-版本历史)):

- **v1.8.2** · 权限/安全修复 + 提交取消撤销快照 + Docker 本地运行说明
...
- 关注事件**不**通知被关注者(保持安静)
- 关注 / 粉丝列表中的签名预览统一转义输出,避免 HTML 注入
...
- `manage_problemset` 只控制题单本身;题单内私有题目的展示和计数仍以题目自身访问权限为准
...
- 支持在收藏列表中快速取消收藏
- 同一用户对同一题只能收藏一次
- 收藏列表只展示当前用户对题目本身有访问权限的题目,避免私有题目通过收藏列表泄漏
...
支持附件上传(单文件 20MB,每工单最多 10 个附件,任意类型),支持 admin 内部备注、用户撤回、5 状态流转、24h 5 个工单频率限制。**回复 / 状态变更触发通知中心通知。**已结案状态不允许继续追加附件,失败上传会自动清理临时文件。
...
- 触发用户名牌子的"作弊者"识别
- 完全可撤销(撤销时恢复状态并反向修正)
- 取消评测会保存 `original_status` / `original_score` / `original_pending` / `original_result` 快照
- 撤销取消时优先恢复原始评测状态并修正 `ac_num`;旧记录没有快照时回退为管理员重评
- 撤销 / 重评入口仅 `manage_judge_action` 可操作,提交者本人不能清除管理员取消标记
...
- 用户可投稿题解,支持 Markdown
- 投稿默认进入审核状态
- 拥有 `manage_problem` / `manage_solution` 的审核者可在 `/admin/solutions` 集中处理
- 审核者可按题禁止/恢复题解投稿;关闭投稿后直接 POST 也会被拦截
- **审核员信息公开化**(v1.6.0):
...
# 7. 创建管理员账号(注册账号后)
docker compose exec -T mariadb mariadb -u root syzoj \
...
自定义业务表已抽出到 `sql/001_custom_tables.sql`,可重复执行。脚本包含 `judge_state_admin_action` 的 `original_status` / `original_score` / `original_pending` / `original_result` 快照字段,并通过 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` 兼容旧库重复迁移。
...
如果 Docker daemon 没有继承 shell 代理,`docker compose up -d` 可能在拉取 Docker Hub 镜像时超时。可先用 `skopeo` 通过当前 shell 代理导入镜像:
...
skopeo copy docker://docker.io/library/mariadb:latest docker-daemon:mariadb:latest
skopeo copy docker://docker.io/library/rabbitmq:3 docker-daemon:rabbitmq:3
skopeo copy docker://docker.io/menci/syzoj-web:latest docker-daemon:menci/syzoj-web:latest
skopeo copy docker://docker.io/menci/syzoj-judge:latest docker-daemon:menci/syzoj-judge:latest
```

如果本机 Docker bridge 创建网卡失败,可临时创建一个不入库的 host network 覆盖文件:

```bash
cat >/tmp/algobeat-host-network.yml <<'YAML'
services:
  web:
    network_mode: host
    ports: []
    ...
    environment:
      ...
      - SYZOJ_WEB_LISTEN_PORT=4567
      - SYZOJ_WEB_DB_HOST=127.0.0.1
  ...
  mariadb:
    network_mode: host
    healthcheck:
      test: ["CMD", "mariadb-admin", "ping", "-h", "127.0.0.1", "-u", "root"]
      start_period: 10s
      interval: 5s
      timeout: 30s
      retries: 5
  ...
  judge-daemon:
    ...
    environment:
      - TZ=Asia/Shanghai
      - SYZOJ_JUDGE_WEB_URL=http://127.0.0.1:4567
      - SYZOJ_JUDGE_RABBITMQ_URI=amqp://127.0.0.1/
      - SYZOJ_JUDGE_TESTDATA_PATH=/app/testdata/testdata
      - SYZOJ_JUDGE_REDIS_URI=redis://127.0.0.1:6379
  judge-runner-1:
    network_mode: host
    environment:
      - TZ=Asia/Shanghai
...
docker compose -f docker-compose.yml -f /tmp/algobeat-host-network.yml up -d
docker compose -f docker-compose.yml -f /tmp/algobeat-host-network.yml exec -T mariadb mariadb -u root syzoj < sql/001_custom_tables.sql
```

host network 方案下 Web 可访问 `http://127.0.0.1:4567/`。如果 `judge-runner-1` 日志出现 `Cgroup not mounted`,说明当前宿主机不满足 simple-sandbox/cgroup 要求;Web 可以先用于页面调试,但真实评测不可用,可临时停止 runner:

```bash
docker compose -f docker-compose.yml -f /tmp/algobeat-host-network.yml stop judge-runner-1
```
...
# 使用本地 host network 覆盖文件启动
docker compose -f docker-compose.yml -f /tmp/algobeat-host-network.yml up -d
docker compose -f docker-compose.yml -f /tmp/algobeat-host-network.yml down

# 查看状态和日志
docker compose logs --tail 100 web
docker compose logs -f web                 # 实时跟随

# 进容器调试
docker compose exec web bash
docker compose exec mariadb mariadb -u root syzoj
```
...
- **v1.8.2**:权限与安全收口·题解审核改用 `manage_problem` / `manage_solution`·提交取消保存原始评测快照并可恢复·`restore-and-rejudge` 收紧到 `manage_judge_action`·站内安全跳转·关注签名转义·题单/收藏私有题目可见性修复·工单结案附件限制·本地 Docker 运行排障文档
